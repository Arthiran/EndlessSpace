...
layout (binding = 15) uniform sampler2D s_CurrentDepth;
...
void main() {
	vec2 motion = CalculateMotion(s_CurrentDepth, a_ViewProjectionInv, a_PrevViewProjection);
	fragColour = CalculateBlur(s_screenTex, motion, inUV);
}